...
/// </summary>
public abstract class AsyncBatchProcessor<TIn, TOut> : AsyncBackgroundService
{
	...
	protected abstract ValueTask<TOut[]> ExecuteBatchProcess(ICollection<TIn> inputs, CancellationToken cancellationToken);

	/// <summary>
	/// Queues <paramref name="value"/> for the next batch and completes when that batch has been processed.
	/// </summary>
	protected async Task<TOut> Process(TIn value,
	...
	private async Task ExecuteReadLoop(CancellationToken stoppingToken)
	{
		...
		{
			while (state._channel.Reader.TryRead(out (TIn Value, TaskCompletionSource<TOut> TaskCompletionSource) item))
			{
