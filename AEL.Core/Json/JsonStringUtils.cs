...
public static partial class StringUtils
{
	...
	private static readonly Regex RegexDelimiter = new(@"^[,:\[\]\{\}\(\)\n]$");
	private static readonly Regex RegexStartOfValue = new(@"^[\[\{\w-]$");
	...
	{
		return code is >= CodeZero and <= CodeNine or >= CodeUppercaseA and <= CodeUppercaseF or >= CodeLowercaseA and <= CodeLowercaseF;
	}
	...
	{
		return code is >= CodeOne and <= CodeNine;
	}
	...
	{
		return code is CodeNonBreakingSpace or >= CodeEnQuad and <= CodeHairSpace or CodeNarrowNoBreakSpace or CodeMediumMathematicalSpace or CodeIdeographicSpace;
	}
	...
	{
		int index = text.LastIndexOf(textToStrip, StringComparison.Ordinal);
		return index != -1
			? text[..index] + (stripRemainingText ? string.Empty : text[(index + 1)..])
			: text;
	}
